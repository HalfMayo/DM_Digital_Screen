...
export default function Slider({min, max, lockable = false, label, value, onChange} : SliderProps) {
        ...
        <div className="flex items-center justify-center">
            ...
            <label htmlFor={label}>{label}:</label>
                <div className="relative">
                    ...
                    onChange={onChange}
                    id={label} name={label}
                    disabled={lockable && locked ? true : false}></input>

                    <output ref={outputRef} className={`absolute -top-full left-2/4 px-2 py-px rounded-full
                    ${lockable && locked ? "bubble-disabled text-on-surface bg-surface-low" : "bubble text-on-primary bg-primary-teal"}`}><span>{value}</span></output>
                </div>
            </div>
