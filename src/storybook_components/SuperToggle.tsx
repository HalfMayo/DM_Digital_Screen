...
export default function SuperToggle({
  info,
  isOpen,
  i,
  onClick,
  ...
  return (
    <li className="list-none" style={{ width: `${width}` }}>
      <div
        ...
        onClick={() => onClick(i)}
        className={`p-4 bg-surface-container text-base m-0 flex items-center justify-between gap-4 w-full ${className}`}
      >
      ...
      <div
        className={`bg-surface-container max-h-0 overflow-hidden transition-[max-height] ease-linear duration-700 ${
          isOpen === i
            ? " max-h-96 transition-[max-height] ease-linear duration-700"
            : ""
        }`}
      >
