    ...
    <div className="flex flex-col gap-4 items-center">
            ...
            className={`flex items-center justify-center text-center rounded-full w-8 h-8 ${
              index >= i.toString()
                ...
                : "bg-disabled text-on-surface"
                                                    ...
                                                    `}
            onClick={() => dispatch({ type: "nextStep", payload: init.name })}
          >
            {init.initiative}
          </div>
        )}
        <textarea
          className="rounded-sm bg-surface-container resize-none p-2 w-24 text-xs scrollbar focus:outline-none"
          value={init.conditions}
          onChange={(e) =>
            dispatch({
              type: "update",
              payload: {
      ...
      <div className="flex gap-2">
        {openEdit === init.name ? (
          <SvgButton
            label="submit"
            onClick={() => dispatch({ type: "changeOrder" })}
            svg={Confirm}
          ...
          <SvgButton
            ...
            onClick={() =>
              dispatch({ type: "editInterface", payload: init.name })
            }
            svg={Edit}
            color="text-on-surface"
          />
        )}
        {!friends.includes(init.name) && (
          <SvgButton
            width="32"
            height="32"
            color="text-on-surface"
            label="Check info"
            svg={Magnifying}
            onClick={() => dispatch({ type: "openInfo", payload: init.name })}
          />
        )}
      </div>
    </div>
